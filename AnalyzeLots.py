...
def main() -> None:
    ...
    if args.file.is_file():
        ...
        if not args.no_summary:
            ...
            print(f"Total short term losses {fmt_dollar(data.loc[date_short & (data['gain'] < 0), 'gain'].sum())}")
            print(f"Total long term losses {fmt_dollar(data.loc[date_long & (data['gain'] < 0), 'gain'].sum())}")
